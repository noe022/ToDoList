...
tasksBackup.forEach(taskText => CreateTask(taskText));

function CreateTask(taskText){
  if(taskText.text.trim() === "") return;
  ...
  task.textContent = taskText.text;
  ...
  deleteTask.addEventListener('click', function(){
    ...
    taskCard.addEventListener('animationend', function(){
      ...
      tasksBackup.forEach(function(oneTaskBackup) {
        if (oneTaskBackup.text !== taskText.text) {
